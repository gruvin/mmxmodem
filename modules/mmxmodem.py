...
class XMODEM(object):
    ...
    def send(self, stream, retry=16, timeout=60, quiet=0, callback=None):
        ...
        while True:
            ...
            error_count += 1
            log.debug("error_count: %d", error_count)
        ...
        while True:
            ...
            # emit packet
            while True:
                log.debug('EMITTING PACKET SIZE:%d, SEQ: %d', packet_size, sequence)
                if packet_size == 128:
                    self.putc(SOH)
                ...
                self.putc(data)
                if crc_mode:
                    log.debug('CRC16: 0x%04x', crc)
                    self.putc(chr(crc >> 8))
                    self.putc(chr(crc & 0xff))
                else:
                    log.debug('CRC8: 0x%02x', crc)
                    self.putc(chr(crc))
                ...
                # protocol error
                self.abort(timeout=timeout)
                log.error('PROTOCOL ERROR. Expecting NAK/ACK. Got 0x%02x', ord(char))
                return False
    ...
    def recv(self, stream, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0):
        ...
        while True:
            ...
            # BJR: Maximite doesn't cupport 16-bit CRC. This is just wasting 8 seconds of our time, so ...
            crc_mode = 0;

            if error_count >= retry:
                self.abort(timeout=timeout)
                return None
            elif crc_mode and error_count < (retry / 8):
        ...
        while True:
            while True:
                if char == SOH:
                    ...
                    # strip all contiguous ^Z's (EOF) off the end
                    i = 0
                    eof_offset = 0;
                    for c in data:
                        log.debug('<:0x%02x', ord(c));
                        if c == EOF: # 0x1a ^Z
                            if (eof_offset == 0): eof_offset = (packet_size - i)
                        else:
                            eof_offset = 0
                        i += 1
                    log.debug('1st EOF at packet[%d]', eof_offset)

                    income_size -= eof_offset
                    stream.truncate(income_size)
                    return income_size
